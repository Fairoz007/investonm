      ...
      <div className="container-custom relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
          <div className="max-w-3xl">
            <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-6 leading-tight font-sans tracking-tight">
              {t('services.labels.thriving')} <span className="text-primary">{t('services.labels.incentives')}</span>
            </h2>
          ...
          >
            {t('services.labels.exploreAll')} <ArrowUpRight className="w-5 h-5 transition-transform ml-2" />
          </Link>
        </div>
        ...
        <div className="flex overflow-x-auto pb-4 mb-12 gap-3 scrollbar-hide snap-x">
          {SECTORS.map((sector, idx) => (
            ...
            >
              {t(`services.items.${sector.id}`) || sector.title}
            </button>
          ))}
        </div>
        ...
        <div className="bg-card rounded-[2rem] border border-border shadow-sm overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-2/5 aspect-[4/3] lg:aspect-auto relative bg-muted">
            <img
              ...
              alt={t(`services.items.${activeSector.id}`) || activeSector.title}
          ...
          <div className="p-8 md:p-12 w-full lg:w-3/5">
            <h3 className="text-3xl font-bold text-foreground mb-4 font-sans tracking-tight">{t(`services.items.${activeSector.id}`) || activeSector.title}</h3>
            ...
            >
              {t('hero.cta.explore')} <ArrowUpRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
      </div>
